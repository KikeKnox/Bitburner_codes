...
export async function main(ns) {
    /*El nuevo archivo solo hara llamadas a otros archivos y hara 
    comprobacion de RAM para asegurar que se pueden correr los archivos*/
    ...
    let myCores = ns.getServer("home").cpuCores;
    let myUsedRam;
	...
	if(ns.args[0]) ns.run("hacknet.js");
	...
	while (!flg) {
		...
		for (n = 0; n < serTemp.length; n++) {
			if (servis.indexOf(serTemp[n]) == -1 && serTemp[n].substring(0,7) != "Server_") {
				...
				if(ns.getServerMoneyAvailable(serTemp[n]) == 0) {
					isFaction.push(true);
				} else {
			...
			for (n = 1; n < isNuked.length; n++) {
				if (numPrograms >= numPorts[n] && !isNuked[n]) {
					...
					if (!isFaction[n]) {
						...
						es mas facil meter esta parte en su propio codigo*n/
					}
				}
			}
